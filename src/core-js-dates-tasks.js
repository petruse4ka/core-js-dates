...
function dateToTimestamp(date) {
  const controlDate = new Date('01 Jan 1970 00:00:00 UTC');
  const testDate = new Date(date);

  return testDate - controlDate;
}
...
function getNextFriday(date) {
  ...
  const dateNumber = finalDate.getDay();
...
function getCountDaysInMonth(month, year) {
  const lastMonthDate = new Date(Date.UTC(year, month, 0));
...
function formatDate(date) {
  ...
  const minutes = String(new Date(date).getUTCMinutes()).padStart(2, '0');
  const seconds = String(new Date(date).getUTCSeconds()).padStart(2, '0');
  const timeConvention = new Date(date).getUTCHours() >= 12 ? 'PM' : 'AM';

  return `${month}/${day}/${year}, ${hours}:${minutes}:${seconds} ${timeConvention}`;
}
...
function getWeekNumberByDate(date) {
  ...
  const dayOfWeek =
    fourthOfJanuary.getDay() === 0 ? 7 : fourthOfJanuary.getDay();
  const firstWeekStart = fourthOfJanuary.setDate(4 + 1 - dayOfWeek);
  const finalDate = new Date(date);

  return Math.ceil(((finalDate - firstWeekStart) / 86400000 + 1) / 7);
}
...
function getQuarter(date) {
  const testMonth = new Date(date).getMonth();

  switch (true) {
    case testMonth >= 0 && testMonth <= 2:
      return 1;
    case testMonth >= 3 && testMonth <= 5:
      return 2;
    case testMonth >= 6 && testMonth <= 8:
      return 3;
    case testMonth >= 9 && testMonth <= 11:
      return 4;
    default:
      return 'wrong test month';
  }
}
...
function isLeapYear(date) {
  ...
  if (testYear % 4 === 0 && testYear % 100 !== 0) return true;
